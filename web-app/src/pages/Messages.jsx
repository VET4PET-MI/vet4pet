...
function Avatar({ name, size = 'md' }) {
  ...
  return (
    <div className={`${sz} bg-brand-soft rounded-full flex items-center justify-center text-brand-dark font-bold shrink-0`}>
      {(name?.[0] ?? '?').toUpperCase()}
    </div>
  )
}
        ...
        <div className="relative bg-brand-tint px-6 py-5 border-b border-brand/15">
          ...
          <div className="relative flex items-center justify-between rtl:flex-row-reverse">
            <div className="flex items-center gap-3 rtl:flex-row-reverse">
              ...
              <h2 className="text-lg font-bold text-brand-deep">{t('messages.newMessage')}</h2>
            </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden">
        <div className="relative bg-brand-tint px-6 py-5 border-b border-brand/15">
          ...
          <div className="relative flex items-center justify-between rtl:flex-row-reverse">
            <div className="flex items-center gap-3 rtl:flex-row-reverse">
              ...
              <h2 className="text-lg font-bold text-brand-deep">{t('messages.messageClinic')}</h2>
            </div>
            <button onClick={onClose} className="p-2 text-slate-500 hover:bg-white/60 rounded-xl transition"><X className="w-5 h-5" /></button>
          </div>
        </div>
        <form onSubmit={handleSend} className="px-6 py-5 space-y-4">
          {loading ? (
            ...
            <div className="flex items-center gap-3 p-3 bg-brand-soft rounded-xl rtl:flex-row-reverse rtl:text-right">
                  ...
                  <li key={conv._id}>
                    <button
                      ...
                      className={[
                        ...
                        activeId === partner.id ? 'bg-brand-soft border-e-2 border-brand' : '',
                      ].join(' ')}
                    >
                      <div className="flex items-center justify-between gap-2 rtl:flex-row-reverse">
                        ...
                        <div className="shrink-0 text-end">
                          ...
                          {conv.unread > 0 && (
                            <span className="inline-block mt-1 min-w-[1.25rem] text-center text-xs font-bold bg-brand text-white rounded-full px-1.5 py-0.5">
                              {conv.unread}
                            </span>
                          )}
                        </div>
                  ...
                  return (
                    <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={[
                        ...
                        isMe
                          ? 'bg-brand text-white rounded-br-sm ms-2'
                          : 'bg-white text-slate-800 border border-slate-200 rounded-bl-sm ms-2',
                      ].join(' ')}>
                        <p>{msg.content}</p>
                        <p className={`text-xs mt-1 ${isMe ? 'text-brand-soft' : 'text-slate-400'}`}>
