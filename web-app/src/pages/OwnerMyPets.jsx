    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-800">Add New Pet</h2>
          <button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-xl"><X className="w-5 h-5" /></button>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Pet Name <span className="text-red-400">*</span></label>
            <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Buddy" required
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" max="30" value={form.age} onChange={e => set('age', e.target.value)} placeholder="e.g. 3"
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="font-semibold text-slate-600">No pets yet</p>
            <p className="text-sm text-slate-400 mt-1 mb-6">Add your first pet to get started.</p>
            <button onClick={() => setAdd(true)}
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-0.5">{pet.breed || '—'}{pet.age ? ` · ${pet.age} yr` : ''}</p>
                  </div>
                  <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-xs text-indigo-600 font-medium">View Records →</span>
                  </div>
                </div>
              )
            })}
